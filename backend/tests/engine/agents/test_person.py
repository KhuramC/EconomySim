...
@mark.parametrize("personal_income_tax, expected_balance", [(1,0), (0.02,490), (0.1,450), (1/3,333.333333333), (0,500)])
def test_income_tax(mock_economy_model, personal_income_tax, expected_balance):
    """
...
def test_purchase_goods(mock_economy_model, food_sales_tax, entertainment_sales_tax, purchased_food_inventory, purchased_entertainment_inventory, total_spent):
    ...
    mock_economy_model.policies["sales_tax"][IndustryType.ENTERTAINMENT] = entertainment_sales_tax
    ...
    # 4. Total spent: 800
    assert total_spent == approx((purchased_food_inventory * (10 * (1 + food_sales_tax))) + (purchased_entertainment_inventory * (20 * (1 + entertainment_sales_tax))))
    assert food_industry.inventory == approx(100 - purchased_food_inventory)
    assert entertainment_industry.inventory == approx(100 - purchased_entertainment_inventory)
